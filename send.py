...
def confirm_attachments():
    ...
    try:
        for filename in os.listdir('ATTACH'):
            ...
            entry = input(f"""TYPE IN 'Y' or 'y' AND PRESS ENTER IF YOU CONFIRM T0 ATTACH {filename} 
                                    TO SKIP ATTACH PRESS ENTER: """)
            confirmed = True if entry == 'Y' or entry == 'y' else False
            if confirmed:
...
def send_emails(server: SMTP, template):
    ...
    for receiver, message in get_msg('data.csv', template):
        ...
        multipart_msg["Subject"] = "[AMU internal] [Urgent/Time Bound] Information required from each and every faculty members."
        ...
        try:
            ...
            x = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p") + " Sent to " + receiver
            f = open("success.log", "a")
            f.write(x + "\n")
            f.close()
        except Exception as err:
            print(f'Problem occurend while sending to {receiver} ')
            print(err)
            y = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p") + " Problem occurend while sending to " + receiver  
            e = open("error.log", "a")
            e.write(y + "\n")
            e.close()
...
if __name__ == "__main__":
    ...
    with open('compose.md', encoding="utf8") as f:
        template = f.read()
